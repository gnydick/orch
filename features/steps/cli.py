...
@given('create service_config "{svc_config_name}" for "{svc}" via cli')
def step_impl(context, svc_config_name, svc):
    expected_svc_cfg_id = '%s:%s' % (svc, svc_config_name)
    context.cli.create({'resource': 'service_config', 'name': svc_config_name, 'service_id': svc})
    svc_cfg = context.rest.get_instance('service_config', expected_svc_cfg_id)
