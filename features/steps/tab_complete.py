...
def step_impl(context, service_id):
    ...
    filtered_service_configs = context.rest.add_service_config_for_service_completer(None, parsed_args)
    assert 'testservice_config1' not in filtered_service_configs
    assert 'testservice_config2' not in filtered_service_configs
    assert 'testservice_config3' in filtered_service_configs
...
def step_impl(context, service_id):
    ...
    filtered_service_configs = context.rest.add_service_config_for_service_completer(None, parsed_args)
    assert 'testservice_config1' not in filtered_service_configs
    assert 'testservice_config2' in filtered_service_configs
    assert 'testservice_config3' in filtered_service_configs
...
def step_impl(context, service_id):
    parser = context.args.assign_args(context.rest)
    parsed_args, unknown_args = parser.parse_known_args(['service', 'add_svc_config', service_id, 'mock'])
    filtered_service_configs = context.rest.del_service_config_for_service_completer(None, parsed_args)
    assert 'testservice_config1' in filtered_service_configs
    assert 'testservice_config2' in filtered_service_configs
    assert 'testservice_config3' not in filtered_service_configs
