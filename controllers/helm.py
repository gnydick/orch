...
from db import db
from models import Deployment, Config
...
class HelmController:
    ...
    def genChart(self, dep_id):
        ...
        helm_files = list(
            map(lambda c: c.id, filter(lambda x: x.config_type.id == 'helm_template', deployment.configs)))
        configs = s.query(Config).filter(Config.id.in_(helm_files)).all()
        print(configs)
