...
class RestController:
    ...
    schema = OrchSchema()
    global endpoint
    endpoint = os.getenv("ORCH_ENDPOINT", "orch.infra.svc.cluster.local")

    def __init__(self):
        pass

    URL = 'http://%s/api/rest/v1/' % endpoint
    ...
    def add_service_config_for_service_completer(self, prefix, parsed_args, **kwargs):
        current_service_configs = self.__collection_for_resource_completer__(prefix, parsed_args, 'service_configs')
        filters = [dict(name='id', op='not_in', val=current_service_configs)]
        filtered_service_configs = [x[0] for x in self._complex_list_('service_config', filters)]
        return filtered_service_configs

    def del_service_config_for_service_completer(self, prefix, parsed_args, **kwargs):
        return self.__collection_for_resource_completer__(prefix, parsed_args, 'service_configs')
                ...
                for key in field.keys():
                    if re.match('^-', key):
                        if field[key]['args']['dest'] in args_dict:
                            json[field[key]['args']['dest']] = args_dict[field[key]['args']['dest']]
                    else:
                        if 'args' in field[key]:
                            if field[key]['args']['dest'] in args_dict:
                                json[key] = args_dict[key]
                        else:
                            if key in args_dict:
                                json[key] = args_dict[key]

        for opt in ['tag', 'defaults']:
            if opt in args_dict:
                json[opt] = args_dict[opt]
        return self._post_rest_(args_dict['resource'], json=json)
    ...
    def query(self, resource, search_field, op, value=None, output_fields=None, resperpage=None, format="text", **kwargs):
        return self._list_(resource, search_field, op, value, output_fields, resperpage, format, **kwargs)

    def list(self, resource):
        return self._list_(resource)

    def _list_(self, resource, search_field=None, op=None, value=None, output_fields=None, resperpage=None,
               format="text", **kwargs):
    ...
    def orch_wrapped_deployment_search(self, prefix, parsed_args, **kwargs):
